...
class MenuSalesTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.menu_vibs_burger_01 = Menu.objects.create(id=1, menu_name='버거', price='10000', group=self.group_vibs_burger)
        self.menu_vibs_burger_02 = Menu.objects.create(id=2, menu_name='불고기버거', price='10000', group=self.group_vibs_burger)
        self.menu_bibigo_01 = Menu.objects.create(id=3, menu_name='제육덮밥', price='10000', group=self.group_bibigo)
        self.menu_bibigo_02 = Menu.objects.create(id=4, menu_name='갈비비빔밥', price='15000', group=self.group_bibigo)
    ...
    def test_menu_crud(self):
        ...
        response = self.client.post('/api/menu', data = menu_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        menu_fail_data = {
            'price': 15000,
            'group': 1
        }
        response = self.client.post('/api/menu', data = menu_fail_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        ...
        response = self.client.get('/api/menu/5')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Menu.objects.filter(menu_name='test_menu').exists(), True)

        patch_data = {
            'price' : 8000
        }
        response = self.client.patch('/api/menu/5', patch_data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Menu.objects.get(id=5).price, 8000)

        response = self.client.delete('/api/menu/5')
