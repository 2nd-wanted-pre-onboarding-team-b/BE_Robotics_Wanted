...
def check_payment_validate(c: CustomedAPITestCase, time_range, time_size,
...
def check_payment_result(c: CustomedAPITestCase, time_range, time_size,
    price_range, party_size, restaurant_group):
    if time_range:
        # str을 datetime으로 변형
        for i in range(len(time_range)):
...
def check_partysize_validate(c: CustomedAPITestCase, time_range, time_size,
    price_range, party_size, restaurant_group):
...
def check_partysize_result(c: CustomedAPITestCase, time_range, time_size,
    price_range, party_size, restaurant_group):
    if time_range:
        # str을 datetime으로 변형
        for i in range(len(time_range)):
            time_range[i] = str2datetime(time_range[i])
    try:
        res: List[Dict[str, object]] = ExtensionLogSearcher.search_by_partysize(
            time_range          = time_range, 
            time_size           = time_size, 
            price_range         = price_range,
            party_size          = party_size, 
